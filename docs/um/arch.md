...
In the latest pulp cluster instance, a two-level clock gating approach is used:
...
![iDMA top Clock gating](../images/iDMA_top_clk_gate.drawio.svg)

Once the transfer has been configured, the clock gating control logic inside the iDMA wrapper makes sure that the datapath is clocked only when needed.