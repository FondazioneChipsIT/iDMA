...
module apb2reg #(
  /// Use combinational feedthrough, without a latched request as intended by the APB specification
  ...
  if (Feedthrough) begin : gen_feedthrough
    // In this mode, directly using the register interface is the simplest solution.
    always_comb begin
      ...
      // FIX: missing in the original source. Without this assignment, 'valid'
      // is not assigned in any branch of the always_comb block, causing an
      // inferred latch. As a consequence, mem_ctrl_reg_wrap would never
    ...
    // Latch the forward path as required by the APB specification.
